...
// Dashboard landing view for a volunteer. Styled to match the landing page and
// help-seeker dashboard: shared design tokens (surface/ink/hairline/forest/pin),
// the Alumni Sans display face on headings, soft card shadows, and the coral
// map-pin accent reserved for urgency and the primary CTA.
//
// Left: greeting, three impact stat pills, and a "Top priority requests"
// preview — the AI-ranked open requests where a volunteer's help is needed most
// (the product's core value: browse the priority feed). Right: a data-driven
// request heat map and the volunteer's upcoming tasks.
//
// @param {object} [currentUser] - to greet the volunteer by name
// @param {object} stats - { completedCount, peopleHelped, skillWays }
// @param {object[]} requests - the AI-prioritized open requests feed
// @param {() => void} onViewRequests - jump to the full Requests view
// @param {{date: string, month: string, title: string}[]} tasks - upcoming tasks
const VolunteerDashboardView = ({ currentUser, stats, requests = [], onViewRequests, tasks }) => {
  const { t } = useTranslation();
  const name = currentUser?.name?.split(' ')[0] || t('volunteer.dashboard.defaultName');

  // The few highest-priority open requests, so the volunteer sees where help is
  ...
  return (
    <div className="grid lg:grid-cols-[1fr_minmax(320px,420px)] gap-6">
      ...
      <div className="bg-surface-2 dark:bg-surface-2 rounded-3xl p-6 sm:p-8 ring-1 ring-hairline shadow-card transition-colors duration-300">
        ...
        {/* Impact stat pills — the volunteer's own activity. */}
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-8">
          <StatPill value={stats.completedCount} label={t('volunteer.dashboard.requestsCompleted')} />
          <StatPill value={stats.peopleHelped} label={t('volunteer.dashboard.peopleHelped')} />
          <StatPill value={stats.skillWays} label={t('volunteer.dashboard.skillWays')} />
        </div>

        {/* Top priority requests — the AI priority feed, previewed. */}
        <div className="flex items-center justify-between mt-8 mb-3">
          <h3 className="font-display text-2xl text-ink tracking-wide">
            {t('volunteer.dashboard.topPriority')}
          </h3>
        ...
        {topRequests.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-hairline p-6 text-center">
            <p className="text-ink font-semibold">{t('volunteer.dashboard.noOpenRequests')}</p>
          </div>
        ) : (
        ...
        {/* Primary CTA — browse the full priority feed (the volunteer's flow). */}
        {onViewRequests && (
          <div className="flex justify-center mt-8">
            ...
            >
              {t('volunteer.dashboard.browseAllRequests')}
            </button>
          </div>
        )}
      </div>

      {/* ---- Right column: data-driven heat map + upcoming tasks ---- */}
      <div className="bg-forest-800 dark:bg-surface-2 rounded-3xl p-6 ring-1 ring-hairline shadow-card transition-colors duration-300">
        <h2 className="font-display text-2xl sm:text-3xl text-white dark:text-forest-300 text-center mb-4 tracking-wide">
          {t('volunteer.dashboard.whereHelpNeeded')}
        </h2>
...
const PriorityRequestRow = ({ request, onView, t }) => {
  const typeLabel = request.category
    ...
    : t('volunteer.dashboard.requestFallback');
